...
void ShellContextMenu::ShowMenu(HWND hwnd, const POINT* pt, IUnknown* site, Flags flags)
{
	...
	if (m_contextMenu)
	{
		if (site)
		{
			auto objectWithSite = m_contextMenu.try_query<IObjectWithSite>();
	...
	if (cmd >= MIN_SHELL_MENU_ID && cmd <= MAX_SHELL_MENU_ID)
	{
		...
		if (!handled)
		{
			std::unique_ptr<std::string> parsingPathOpt = MaybeGetFilesystemDirectory();
...
std::unique_ptr<std::string> ShellContextMenu::MaybeGetFilesystemDirectory() const
{
	...
	if (SUCCEEDED(hr))
	{
		// 转换宽字符指针为 std::wstring
		std::wstring wstr(parsingPath.get());

		// 使用 wstring 构造 std::string
		std::string str(wstr.begin(), wstr.end());

		// 创建 std::unique_ptr<std::string> 来管理 std::string
		std::unique_ptr<std::string> ptr(new std::string(str));
		return ptr;
	}
