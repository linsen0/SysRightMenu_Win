...
namespace MenuHelper
{
	...
	void RemoveAdjacentSeparators(HMENU menu)
	{
		int count = GetMenuItemCount(menu);
		if (count == -1)
		{
			std::wcerr << L"GetMenuItemCount failed with error code: " << GetLastError() << std::endl;
			return;
		}
		...
		for (int i = count - 1; i >= 0; i--)
		{
			...
			menuItemInfo.cbSize = sizeof(menuItemInfo);
			menuItemInfo.fMask = MIIM_FTYPE;
			auto name = GetMenuItemString(menu, i, true);
			if (!GetMenuItemInfo(menu, i, TRUE, &menuItemInfo))
			{
				std::wcerr << L"GetMenuItemInfo failed with error code: " << GetLastError() << std::endl;
				continue;
			}
			...
			if (previousItemSeparator && currentItemSeparator)
			{
				...
				{
					std::wcerr << L"DeleteMenu failed with error code: " << GetLastError() << std::endl;
					continue;
				}
			}
	...
	int FindMenuItem(HMENU hMenu, const wchar_t* itemName)
	{
		int count = GetMenuItemCount(hMenu);
		for (int i = 0; i < count; i++) {
			auto name = MenuHelper::GetMenuItemString(hMenu, i, true);
