...
std::unique_ptr<std::wstring> GetFolderPathForDisplay(PCIDLIST_ABSOLUTE pidl);
...
std::unique_ptr<std::wstring> TransformUserEnteredPathToAbsolutePathAndNormalize(
	const std::wstring& userEnteredPath, const std::wstring& currentDirectory,
	EnvVarsExpansion envVarsExpansionType);
std::unique_ptr<std::wstring> MaybeExtractPathFromFileUrl(const std::wstring& url);
std::unique_ptr<std::wstring> ExpandEnvironmentStringsWrapper(const std::wstring& sourceString);
std::unique_ptr<std::wstring> MaybeTransformRootPathToAbsolutePath(const std::wstring path,
	const std::wstring& currentDirectory);
std::unique_ptr<std::wstring> PathCanonicalizeWrapper(const std::wstring& path);
std::unique_ptr<std::wstring> PathAppendWrapper(const std::wstring& path,
	const std::wstring& pathToAppend);
std::unique_ptr<std::wstring> PathStripToRootWrapper(const std::wstring& path);
std::unique_ptr<std::wstring> GetCurrentDirectoryWrapper();
